...
- Guix: ~$ guix install emacs-vterm~
  ...
  =emacs-vterm= provides a pre-built =vterm-module.so=, so you don't have to compile
  it yourself, but you'll need to symlink it somewhere in Emacs' ~load-path~. E.g.
  #+begin_src sh
