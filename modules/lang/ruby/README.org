...
* Description
This module add Ruby and Ruby on Rails support.
...
+ =+lsp= Enables LangServer support for this module. You must have =:tools lsp=
  enabled for this to work, as well as the langserver (solargraph) installed on your system.
+ =+rvm= Rvm (Ruby Version Manager) integration.
+ =+rbenv= Rbenv integration.
+ =+rails= Rails navigation commands and server \ console integration.
...
| ~rspec-verify~ | =SPC m t f=      | Runs rspec for the item on cursor  |
