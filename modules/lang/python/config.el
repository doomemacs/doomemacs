...
;; Fix Emacs 28.1 syntax higlighting

(when EMACS28+
  (defun fixed-python-font-lock-assignment-matcher (regexp)
    (lambda (limit)
      (cl-loop while (re-search-forward regexp limit t)
               unless (or (python-syntax-context 'paren)
                          (equal (char-after) ?=))
               return t)))
  (advice-add   'python-font-lock-assignment-matcher :override #'fixed-python-font-lock-assignment-matcher)

  (defun fixed-python-rx (&rest regexps)
              ...
              (coding-cookie (seq line-start ?# (* space)
                                  (or
                                   ...
                                   (: "vim:" (* space) "set" (+ space)
                                    ...
                                    (* space) ":")))))
       (rx ,@regexps)))
  (advice-add  'python-rx :override #'fixed-python-rx))
