...
;;;###autoload
(defun +ivy-colors-action (string)
...
;;;###autoload
(defun +ivy--hex-colors-candidates ()
  "Return a list of `+ivy-hex-colors' candidates."
  (let ((matches))
    (save-match-data
      (save-excursion
...
(defun +ivy/hex-colors ()
  ...
  (let* ((colors (+ivy--hex-colors-candidates))
    ...
    (ivy-read
     ...
     :require-match t
     :history '+ivy-hex-colors-history
     :sort t
     :action #'+ivy--hex-colors-action)))
