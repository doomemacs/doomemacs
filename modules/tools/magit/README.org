...
- +childframe ::
  Display magit (transient.el) prompts in a [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Child-Frames.html][child frame]] rather than the minibuffer. *Requires GUI Emacs.*
...
- [[doom-package:transient-posframe]] if [[doom-module:+childframe]]
- [[doom-package:magit]]
...
- [[doom-package:transient-posframe]] currently [[https://github.com/yanghaoxie/transient-posframe/issues/5][doesn't set the correct height]] in some
  versions of emacs, so =transient-posframe--show-buffer= is overridden to set the
  height correctly.
