...
[[https://github.com/doomemacs/doomemacs/issues/4876][issue #4876]])
#+begin_src sh
...
2. Add =C:\Users\USERNAME\.config\emacs\bin= to your ~PATH~.

   This way, you don't have to type all of =C:\Users\USERNAME\.config\emacs\bin\doom=
   ...
   ~C:\Users\username\.config\emacs\bin:pathA:pathB:pathC~
   #+end_quote
...
=doom install= will set up your =DOOMDIR= at =~/.config/doom= (if it doesn't already
...
#+begin_quote
I recommend you add =~/.config/emacs/bin= to your ~PATH~ so you can call =doom=
directly and from anywhere. Accomplish this by adding this to your .bashrc or
.zshrc file: ~export PATH="$HOME/.config/emacs/bin:$PATH"~
#+end_quote
...
=doomemacs/doomemacs= has been cloned to =~/.config/emacs=):

#+BEGIN_SRC bash
# So we don't have to write ~/.config/emacs/bin/doom every time
...
cp ~/.config/emacs/init.example.el ~/.config/doom/init.el
cp ~/.config/emacs/core/templates/config.example.el ~/.config/doom/config.el
cp ~/.config/emacs/core/templates/packages.example.el ~/.config/doom/packages.el

# You might want to edit ~/.config/doom/init.el here and make sure you only have the
...
To understand the purpose of the =~/.config/doom= directory and =~/.config/doom/init.el=
...
2. Move aside any existing config and install Chemacs2 as your new =~/.config/emacs=:
   #+BEGIN_SRC bash :eval no
   [ -f ~/.emacs ] && mv ~/.emacs ~/.emacs.bak
   [ -d ~/.config/emacs ] && mv ~/.config/emacs ~/.emacs.legacy
...
module in your ~doom!~ block (in =~/.config/doom/init.el=) and press =K= to jump to
...
=~/.config/emacs=. All your customization should be kept in your =DOOMDIR= (e.g.
=~/.config/doom=). Read the [[#Configure][Configure]] section for more on configuring Doom.
#+end_quote
...
+ ~doom build~ will recompile all your installed packages, which is necessary
  ...
  =~/.config/emacs/.local=, then run ~doom sync~.
...
+ Doom does not use =package.el= to manage its packages, but ~use-package~ does!
  ...
  =~/.config/doom/packages.el= to install your packages.
...
;; Relative paths are expanded to ~/.config/emacs/.local/straight/repos/{local-repo}
;; or ~/.config/doom/{local-repo} -- the first that is found.
(package! my-package
  :recipe (:local-repo "my/package"))  ; looks for ~/.config/doom/my/package/my-package.el
...
;; your DOOMDIR. e.g. ~/.config/doom/lisp/package
...
=~/.config/doom/modules/abc/xyz=, then add =:abc xyz= to your ~doom!~ block in
=~/.config/doom/init.el= to enable it.
...
1. Add a ~autoload~ call to Doom's autoload file (found in
   =~/.config/emacs/.local/autoloads.el=, which is read very early in the startup
   process).
...
keyword and adding a ~(package! PACKAGENAME)~ to =~/.config/doom/packages.el= (and
...
- =C-c C-p= :: This launches "vanilla Doom+". That is, Doom core plus the
  ...
  (in =~/.config/doom/init.el=). This *does not* load your private config, however.
