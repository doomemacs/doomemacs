...
Some notes from and for users migrating from Spacemacs:

+ One can achieve behavior similar to [[https://www.spacemacs.org/doc/DOCUMENTATION.html#hybrid][hybrid editing style]] by putting
  ~(setq evil-disable-insert-state-bindings t)~ into =~/.doom.d/init.el=
  file (needs to be set early in the init process to take effect).
